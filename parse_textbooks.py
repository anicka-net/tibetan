...
TEXT_CORRECTIONS = [
    ...
    ('ས\u0FB1', 'ས'),                 # ~193 (སྱོང→སོང, སྱོན→སོན, etc.)
    ('འ\u0FB1', 'འ'),                 # ~162 (འྱི→འི, པའྱི→པའི, etc.)
    ('ཞ\u0FB1', 'ཞ'),                 # ~117 (ཞྱེས→ཞེས, ཞྱིག→ཞིག, etc.)
    ('ཆ\u0FB1', 'ཆ'),                 # ~103 (མཆྱེད→མཆེད, ཆྱེན→ཆེན, etc.)
    ('ལ\u0FB1', 'ལ'),                 # ~79 (ལྱོན→ལོན, ལྱེན→ལེན, etc.)
    ('ཤ\u0FB1', 'ཤ'),                 # ~70 (ཤྱེས→ཤེས, ཤྱོག→ཤོག, etc.)
    ('ཉ\u0FB1', 'ཉ'),                 # ~56 (ཉྱོ→ཉོ, ཉྱིད→ཉིད, etc.)
    ('ཅ\u0FB1', 'ཅ'),                 # ~36 (ཅྱི→ཅི, གཅྱིག→གཅིག, etc.)
    ('ཟ\u0FB1', 'ཟ'),                 # ~31 (ཟྱེར→ཟེར, ཟྱས→ཟས, etc.)
    ('ན\u0FB1', 'ན'),                 # ~25 (ནྱིད→ནིད, ནྱོར→ནོར, etc.)
    ('ཐ\u0FB1', 'ཐ'),                 # ~22 (ཐྱོན→ཐོན, མཐྱོ→མཐོ, etc.)
    ('ང\u0FB1', 'ང'),                 # ~10
    ('ཝ\u0FB1', 'ཝ'),                 # ~4
    ('ཨ\u0FB1', 'ཨ'),                 # ~4
    ('ཏ\u0FB1', 'ཏ'),                 # ~3
    ...
    for key, lines in sorted(lessons_raw.items()):
        ...
        lessons.append({
            ...
            'fillBlanks': [fb for fb in fill_blanks if fb.get('answer')
                          and 'སློབ་སི' not in fb.get('sentence', '')],
        })
...
# ===== FILL-IN ANSWERS =====
# Loaded from fill_answers.json — corpus-solved answers for fill-in-blank exercises.
# Generated by matching word bank options against textbook corpus bigrams.

def _load_fill_answers():
    """Load pre-computed fill-in-blank answers."""
    path = Path(__file__).parent / 'fill_answers.json'
    if not path.exists():
        return {}
    with open(path, 'r', encoding='utf-8') as f:
        return json.load(f)
...
def apply_fill_answers(fill_blanks):
    """Apply pre-computed answers from fill_answers.json to exercises."""
    applied = 0
...
# Specific term corrections: wrong_term -> corrected_term
# These fix cases where the parser captured extra words or split wrong.
VOCAB_TERM_FIXES = {
    'སོལ་ཡོད།': 'སོལ།',                    # "custom exists" → "custom"
    'དགོས་ཀྱྲི་རེད།': 'དགོས།',              # "it is needed" → "need"
    'ཆགས་ཀྩི་རེད།': 'ཆགས།',               # "it is formed" → "to form/become"
    'དང་པོ་རེད།': 'དང་པོ།',                # "is the first" → "first"
    'ཁོ་ན་རེད།': 'ཁོ་ན།',                   # "is only" → "only"
    'ཡོད་མ་རེད།': 'ཡོད་མ་རེད།',            # keep as-is (valid negation pattern)
    'ཐོད་པ་གཏུགས་': 'ཐོད་པ་གཏུགས་པ།',     # truncated → touching foreheads
    'རི་འཛེགས་རུ་': 'རི་འཛེགས་རུ་ཁག',      # truncated → mountaineering team
    'ཤེས་བ་ཡོན་': 'ཤེས་བྱ་ཡོན་ཏན།',        # truncated → education/knowledge
    'ཆུང་དུས་ཀྱི་': 'ཆུང་དུས།',             # "of childhood" → "childhood"
    'དྲང་པོ་དྲང་': 'དྲང་པོ།',               # truncated → "honest"
    'ཉེ་དགའ་ཕོགས་': 'ཉེ་དགའ་ཕོགས།',        # trailing tsheg → "favoritism"
}

# Copula verbs that shouldn't end a vocab term (unless the whole term IS the verb)
...
def clean_vocab(vocab_list):
    ...
    for v in vocab_list:
        ...
        if term.endswith('་') and not term.endswith('།'):
            v['bo'] = term.rstrip('་')

        # Filter out sentence fragments: 5+ syllables ending with copula verb
        term_clean = v['bo'].rstrip('།').rstrip('་')
        syllables = len(term_clean.split('་'))
        if syllables >= 5:
            # Long items are almost certainly sentence fragments
            continue

        # Filter 3-4 syllable items ending with copula (likely sentence not vocab)
        if syllables >= 3:
            is_sentence = False
            for ending in _COPULA_ENDINGS:
                if term_clean.endswith(ending):
                    is_sentence = True
                    break
            if is_sentence:
                continue
...
def main():
    ...
    total_fill = sum(len(l['fillBlanks']) for l in all_lessons)
    fill_with_answer = sum(1 for l in all_lessons for fb in l['fillBlanks'] if fb.get('answer'))
    print(f"Total fill-in-blank exercises: {total_fill}")
    print(f"  With answers: {fill_with_answer}")
    print(f"  Practice (no answer): {total_fill - fill_with_answer}")
